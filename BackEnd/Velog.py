...
def process_pdf(pdf_path):
    ...
    return full_content, {}  # image_details 대신 빈 딕셔너리 반환
...
def get_summary_title_body_tags(processed_text):
    ...
    """
    prompt = f"""
당신은 PDF 문서 분석 능력이 뛰어난 기술 블로그 전문가입니다. 아래의 원본 텍스트를 분석하여, 독자들이 이해하기 쉬운 전문가 수준의 기술 블로그 포스트를 마크다운 형식으로 가독성 좋게 작성해주세요. 원본 텍스트에는 이미지의 OCR 결과가 '--- 이미지 N 시작/끝 ---' 형태로 포함되어 있습니다.
...
본문: 원본 텍스트와 이미지 OCR 결과를 바탕으로, 상세하고 깊이 있는 기술 블로그 본문을 작성해줘. 1.문제 정의 2. 어떤 상황인지 정리 3. 원인 분석 4. 해결 과정과 적용 5. 결과 검증 및 교훈 으로 작성해줘. 특히, 내용의 흐름에 맞게 이미지가 들어갈 가장 적절한 위치에 '[IMAGE_1]', '[IMAGE_2]'와 같은 플레이스홀더를 반드시 삽입해줘.
...
def post_from_pdf():
    try:
        ...
        processed_text, image_details = process_pdf(temp_pdf_path)  # image_details는 빈 딕셔너리
